...
const doorKeywordsByType: Record<string, string[]> = {
  flush: ["FLUSH"],
  teak: ["WOOD", "TEAK", "WOODEN"],
  wooden: ["WOOD", "TEAK", "WOODEN"],
  wpc: ["WPC", "UPVC", "PVC"],
  glassdoor: ["GLASS"],
  glasspanel: ["GLASS"],
  stile: ["STILE", "ALUMINIUM", "ALUMINUM"],
};
...
const materialMatchesDoor = (m: any, doorType: string) => {
  ...
  return kws.some(
    (kw) =>
      prod.includes(kw) || name.includes(kw) || cat.includes(kw) || sub.includes(kw)
  );
};
...
export default function DoorsEstimator() {
  ...
  const getAvailableDoorTypes = () => {
    ...
    const availableProducts = Array.from(new Set(storeMaterials.map(m => m.product).filter(Boolean)));
  ...
  const getAvailableMaterials = () => {
    if (!doorType || !storeMaterials.length) return [];

    // Step 1: filter by door type via DB product field (and safe fallbacks)
    const doorMatched = storeMaterials.filter((m) => materialMatchesDoor(m, doorType));
...
// Door type to product name mapping
const DOOR_TYPE_TO_PRODUCT: Record<string, string> = {
  ...
  "teak": "Wooden door",
  "wpc": "Flush door", // WPC doors can use flush door materials
  "wpc-door": "Flush door",
  "glassdoor": "Flush door", // Glass doors might use flush door frame materials
  "glass-door": "Flush door",
  "stile": "Flush door", // Stile doors use similar materials
  "stile-door": "Flush door",
  "glasspanel": "Flush door",
};



// Get current door config (now simplified since we don't use DOOR_CONFIG)
const getCurrentDoorConfig = () => {
  if (!doorType) return null;
  const productName = DOOR_TYPE_TO_PRODUCT[doorType] || doorType;
  return { label: `${productName} Door`, productName };
};
  ...
  const calculateQuantity = (materialName: string, materialUnit: string): number => {
    const c = count || 1;
    const h = height || 7;
    const w = width || 3;
    ...
    else if (name.includes("door") && (name.includes("panel") || name.includes("shutter") || name.includes("flush") || name.includes("wooden"))) {
      quantity = c;
    }
    // Hinges
    ...
    else if (name.includes("lock") || name.includes("handle") || name.includes("stopper") || name.includes("bolt")) {
      quantity = c;
    }
    // Glass materials
  ...
  const getMaterialsWithDetails = (): MaterialWithQuantity[] => {
    ...
    return selectedMaterials
      .map((selection) => {
        const base = availableMaterials.find((m) => m.id === selection.materialId);
        if (!base) return null;

        // ✅ NEW: find exact DB row matching product+name+brand+shop
        const chosen = storeMaterials.find((m) => {
        ...
        const override = editableMaterials[chosen.id] || editableMaterials[base.id];
        const computedQty = calculateQuantity(chosen.name, chosen.unit);

        const quantity = override?.quantity ?? computedQty;
        const rate = override?.rate ?? chosen.rate ?? 0;
  ...
  const handleToggleMaterial = (materialId: string) => {
    ...
    const variants = storeMaterials
      .filter((m) => normText(m.product) === normText(material.product) && normText(m.name) === normText(material.name))
      .map((m) => ({
    ...
    setSelectedMaterials((prev) =>
      prev.map((m) =>
        m.materialId === materialId
          ? { ...m, selectedBrand: newBrand, selectedShopId: bestShopId || m.selectedShopId }
          : m
      )
    );
  };
      ...
      .set({
        ...
        html2canvas: {
          ...
          ignoreElements: (el: HTMLElement) => {
            ...
            return el.className?.toString().includes("bg-") ||
                  ...
                  <div className="space-y-3 max-h-64 overflow-y-auto border rounded-lg p-4">
                    {availableMaterials.length === 0 ? (
                      <div className="p-4 text-sm text-yellow-700 bg-yellow-50 border border-yellow-200 rounded">
                        ...
                        <div>Door Type: {doorType}</div>
                        <div>Materials in store: {storeMaterials.length}</div>
                        <div className="mt-2">Available products in DB: {Array.from(new Set(storeMaterials.map(m => m.product).filter(Boolean))).join(", ")}</div>
                        <div className="mt-2">Tip: Ensure materials in the database have the correct product field (e.g., "Flush door", "Wooden door") matching the door type.</div>
                      </div>
                    ) : (
                      availableMaterials.map((mat) => {
                        ...
                        const selected = selectedMaterials.find((x) => x.materialId === mat.id);
                        const selectedBrand = selected?.selectedBrand || availableBrands[0] || "Generic";
                    ...
                    {getMaterialsWithDetails().length > 0 ? (
                      <>
                        ...
                        {getMaterialsWithDetails().map((mat) => {
                          ...
                          const material = storeMaterials.find((m) => m.id === mat.id);
                          return (
                            <div key={mat.id} className={cn("p-3 border rounded grid grid-cols-9 items-center")}>
                              ...
                              <div className="col-span-1 text-center font-semibold">{selection?.selectedBrand || material?.brandName || "-"}</div>
              ...
              {step === 8 && (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-4 justify-end pt-4">
                    ...
                    <Button onClick={() => setStep(9)} disabled={getMaterialsWithDetails().length === 0}>
                    ...
                    <table className="min-w-full border-collapse text-sm">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="border px-2 py-2 text-center" colSpan={2}>Rate</th>
                          <th className="border px-2 py-2 text-center" colSpan={2}>Amount</th>
                        </tr>
                        <tr className="bg-gray-50">
                          ...
                          <th className="border px-2 py-1"></th>
                          <th className="border px-2 py-1"></th>
                          <th className="border px-2 py-1">Supply</th>
                          <th className="border px-2 py-1">Install</th>
                          <th className="border px-2 py-1">Supply</th>
                          <th className="border px-2 py-1">Install</th>
                        </tr>
                      </thead>
                      ...
                      <tbody>
                        {getMaterialsWithDetails().map((m, i) => {
                          const supplyRate = m.rate;
                          const installRate = 0;

                          const supplyAmt = m.quantity * supplyRate;
                          const installAmt = m.quantity * installRate;
                          ...
                          return (
                            <tr key={m.id}>
                              ...
                              <td className="border px-2 py-1 text-center">{m.quantity}</td>

                              <td className="border px-2 py-1 text-right">₹{supplyRate}</td>
                              <td className="border px-2 py-1 text-right">₹{installRate}</td>
