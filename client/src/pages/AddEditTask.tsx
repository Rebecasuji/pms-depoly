...
export default function AddEditTask() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/projects/${form.projectId}/members`)
      .then(r => r.json())
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          setEmployees(data);
        } else {
          // If project has no members, fallback to all employees
          setEmployees(allEmployees);
        }
      })
      .catch(() => {
    ...
    apiFetch(`/api/task/${taskId}`)
      ...
      .then((task: any) => {
        ...
        setForm({
          ...
          reminderFrequency: task.reminderFrequency || "4 times",
        });
