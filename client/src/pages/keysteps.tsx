...
export default function KeySteps() {
  ...
  const [companyFilter, setCompanyFilter] = useState<string>(() => localStorage.getItem("keysteps_companyFilter") || "all");
  ...
  const fetchTasksForStep = async (keyStepId: string, projectId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        filtered.forEach(async (task: any) => {
  ...
  useEffect(() => {
    localStorage.setItem("keysteps_companyFilter", companyFilter);
  }, [companyFilter]);
  ...
  const parentsSorted = useMemo(() => {
    ...
    if (companyFilter !== "all") {
      ...
      const projectCompany = (selectedProject?.company || "").toLowerCase();
      
      // If project has no company, we only show it when filter is "all" 
      // OR if the user explicitly selected it and we want to be lenient.
      // But let's stay strict to the filter UNLESS the project has no company and we want to allow it?
      // Actually, if a project has no company, it's "Internal" and should probably show in "All" only.
      if (selectedProject && projectCompany !== companyFilter.toLowerCase()) {
        filtered = [];
      }
    }
                  ...
                  {projects.length > 0 ? (
                    projects
                      .filter((p: any) => {
                        ...
                        return String(p.company || "").toLowerCase() === companyFilter.toLowerCase();
                      })
                      .map((p: any) => (
                    ...
                    <>
                        ...
                        <td className="px-4 py-2 border-r" colSpan={6}>
                          <div className="flex items-center justify-between">
                            <span className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-blue-700">
                              <ListTodo size={13} /> Tasks ({(stepTasks[step.id] || []).length})
                            </span>
                            <button
                              onClick={() => {
                      ...
                      {showTaskFormFor === step.id && (
                        <tr className="border-b bg-blue-50/70">
                          <td className="px-3 py-2 border-r" />
                          <td className="px-4 py-2 border-r" colSpan={6}>
                            <div className="flex flex-wrap gap-2 items-center bg-white border border-blue-200 rounded-lg px-3 py-2">
                              <input
                              ...
                              <div className="flex flex-col gap-1 min-w-[200px]">
                                ...
                                {newTaskForm.taskMembers.length > 0 && (
                              ...
                              <button onClick={() => addTaskToKeyStep(step.id)} className="bg-blue-600 text-white px-3 py-1 rounded text-xs font-semibold">Save</button>
                            ...
                            <tr className="border-b bg-white hover:bg-slate-50 transition-colors">
                              <td className="px-3 py-2 text-center border-r">
                                <button
                                  onClick={() => {
                                    ...
                                    setExpandedTasks((prev) => isTaskExpanded ? prev.filter((x) => x !== task.id) : [...prev, task.id]);
                              ...
                              <td className="px-4 py-2 border-r">
                                <div className="flex items-center gap-2 pl-4">
                                  ...
                                  <div>
                                    <p className="text-[10px] text-blue-500 font-semibold uppercase tracking-wider">Task</p>
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                              ...
                              <td className="px-3 py-2 text-center border-r">
                                <Badge variant="outline" className={`text-xs whitespace-nowrap ${getTaskStatusColor(task.status)}`}>
                                  {task.status || "—"}
                                </Badge>
                              </td>
                              ...
                              <td className="px-3 py-2 text-center">
                                <button
                                  onClick={() => {
                                    setShowSubtaskFormFor(showSubtaskFormFor === task.id ? null : task.id);
                                    setNewSubtaskForm({ title: "", startDate: "", endDate: "" });
                                    if (!isTaskExpanded) {
                                      setExpandedTasks((prev) => [...prev, task.id]);
                                      fetchSubtasksForTask(task.id);
                                    }
                                  }}
                                  className="text-xs text-emerald-600 border border-emerald-200 rounded px-1.5 py-0.5 hover:bg-emerald-50"
                                  title="Add Subtask"
                                >
                                  + Sub
                                </button>
                              ...
                              <>
                                ...
                                {showSubtaskFormFor === task.id && (
                                  <tr className="border-b bg-emerald-50/60">
                                    ...
                                    <td className="px-4 py-2 border-r" colSpan={6}>
                                      <div className="flex flex-wrap gap-2 items-center bg-white border border-emerald-200 rounded-lg px-3 py-2 ml-8">
                                        <input
                                          placeholder="Subtask title *"
                                          className="border rounded px-2 py-1 text-xs flex-1 min-w-[160px]"
                                          value={newSubtaskForm.title}
                                          onChange={(e) => setNewSubtaskForm((p) => ({ ...p, title: e.target.value }))}
                                        />
                                        ...
                                        <button onClick={() => addSubtaskToTask(task.id, step.id)} className="bg-emerald-600 text-white px-3 py-1 rounded text-xs font-semibold">Save</button>
                                ...
                                {subs.map((sub: any) => (
                                  <tr key={sub.id} className="border-b bg-emerald-50/20 hover:bg-emerald-50/50">
                                    <td className="border-r" />
                                    <td className="px-4 py-2 border-r" colSpan={3}>
                                      <div className="flex items-center gap-2 pl-10">
                                        <button
                                          ...
                                          title={sub.isCompleted ? "Mark incomplete" : "Mark complete"}
                                        >
                                    ...
                                    <td className="px-3 py-2 text-center text-xs text-slate-500 border-r">{formatDate(sub.endDate)}</td>
                                    <td colSpan={2} />
                                  </tr>
                                ))}
                              </>
