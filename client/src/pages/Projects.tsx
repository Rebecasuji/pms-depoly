...
export default function Projects() {
  ...
  const [clientFilter, setClientFilter] = useState<string>(() => localStorage.getItem("projects_clientFilter") || "all");
  const [employeeFilter, setEmployeeFilter] = useState<string>(() => localStorage.getItem("projects_employeeFilter") || "all");
  const [searchTerm, setSearchTerm] = useState(() => localStorage.getItem("projects_searchTerm") || "");
  const [statusFilter, setStatusFilter] = useState<string>(() => localStorage.getItem("projects_statusFilter") || "all");
  ...
  const [departmentFilter, setDepartmentFilter] = useState<string>(() => localStorage.getItem("projects_departmentFilter") || "all");
  ...
  useEffect(() => {
    localStorage.setItem("projects_employeeFilter", employeeFilter);
  }, [employeeFilter]);
  ...
  useEffect(() => {
    localStorage.setItem("projects_departmentFilter", departmentFilter);
  }, [departmentFilter]);
  ...
  const filteredProjects = projects.filter(p => {
    ...
    // 6. Employee / Assignee Filter
    let matchesEmployee = true;
    if (employeeFilter !== "all") {
      const emp = employees.find(e => String(e.id) === employeeFilter);
      const empDept = emp ? normalizeDept(emp.department) : "";
      
      const isAssigned = (p.team && p.team.some((tid: string) => String(tid) === employeeFilter)) || String(p.createdByEmployeeId) === employeeFilter;
      const isDeptRelated = empDept && projectDepts.some((dept: string) => dept === empDept);
      
      matchesEmployee = isAssigned || isDeptRelated;
    }

    // 7. Start Date & End Date filters
    ...
    if (startDate) {
      if (!p.startDate) {
        ...
        if (pDate < startDate) matchesStartDateRange = false;
      }
    }
    ...
    if (endDate) {
      if (!p.endDate) {
        ...
        if (pDate > endDate) matchesEndDateRange = false;
      }
    }
    ...
    return matchesSearch && matchesStatus && matchesDepartment && matchesClient && matchesCustom && matchesEmployee && matchesStartDateRange && matchesEndDateRange && matchesOverdue;
  });
  ...
  const sortedProjects = [...filteredProjects].sort((a, b) => {
    const key = sortKey;
    let va: any = a[key === 'name' ? 'title' : (key === 'created' ? 'createdAt' : (key === 'start' ? 'startDate' : 'endDate'))];
    let vb: any = b[key === 'name' ? 'title' : (key === 'created' ? 'createdAt' : (key === 'start' ? 'startDate' : 'endDate'))];
    if (!va) va = "";
    if (!vb) vb = "";
    if (key === 'created' || key === 'end' || key === 'start') {
  ...
  const processedProjects = sortedProjects.map(p => {
    const isAssigned = employeeFilter !== "all" && ((p.team && p.team.some((tid: string) => String(tid) === employeeFilter)) || String(p.createdByEmployeeId) === employeeFilter);
    const emp = employees.find(e => String(e.id) === employeeFilter);
    const empDept = emp ? normalizeDept(emp.department) : "";
    const projectDepts = Array.isArray(p.department)
      ? p.department.map((d: string) => normalizeDept(d))
      : [normalizeDept(String(p.department))];
    const isDeptRelated = employeeFilter !== "all" && empDept && projectDepts.some((dept: string) => dept === empDept);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          {(searchTerm || clientFilter !== "all" || departmentFilter !== "all" || statusFilter !== "all" || employeeFilter !== "all" || startDate || endDate || customFilters.length > 0) && (
            <Button 
              variant="ghost" 
              size="sm" 
      ...
      <div className="grid gap-3 font-sans">
        ...
        {filteredProjects.length === 0 ? (
          ...
          return sortedProcessedProjects.map(project => {
            ...
            if (employeeFilter !== "all" && project._group !== lastGroup) {
                                    ...
                                    <Command>
                                      ...
                                      <CommandList>
                                        ...
                                        <CommandGroup heading="Project Departments">
                                          {["HR", "Operation", "Software Developer", "Finance", "Purchase", "Presales", "IT Support", "Sales"].map((dept) => {
